...
The application's behavior, such as NATS connection parameters and logging levels, can be configured through a combination of a YAML file, environment variables, and sensible defaults. The configuration logic is managed by `src/deepthought/config.py`.

**Configuration File:**
*   A `config.yml` file located in the project root directory is used for base configuration.
*   **Structure**:
    ```yaml
    # Example config.yml
    nats:
      url: "nats://custom_host:4222"
    ...
    *   `nats`: Contains NATS client settings.
        ...
        *   `stream_name`: The default JetStream stream name to be used or created.
    *   `logging`: Contains logging settings.
        *   `level`: The application-wide logging level (e.g., `DEBUG`, `INFO`, `WARNING`, `ERROR`).

**Order of Precedence:**

The final configuration values are determined in the following order (highest precedence first):

1.  **Environment Variables**:
    *   `DTR_NATS_URL`: Overrides the NATS server URL. (e.g., `export DTR_NATS_URL="nats://another_host:4222"`)
    *   `DTR_NATS_STREAM_NAME`: Overrides the NATS JetStream stream name. (e.g., `export DTR_NATS_STREAM_NAME="another_stream"`)
    *   `DTR_LOGGING_LEVEL`: Overrides the logging level. (e.g., `export DTR_LOGGING_LEVEL="DEBUG"`)
2.  **Values in `config.yml`**: If the `config.yml` file exists and contains relevant settings, these will override the defaults.
3.  **Hardcoded Defaults**: Defined in `src/deepthought/config.py`. These are used if no other configuration is provided.
    *   Default NATS URL: `nats://localhost:4222`
    *   Default Stream Name: `deepthought_events`
    *   Default Logging Level: `INFO`

This layered approach provides flexibility for different deployment environments and development setups. For example, you can use the default `config.yml` for local development and override specific settings with environment variables in a production or testing environment.
...
The end-to-end tests, which may involve Python and C# components interacting (e.g., via `tests/test_end_to_end.py` using the `tests/e2e_csharp_client`), require the C# test client to be built first.

To build the C# client:
1.  Ensure you have the .NET SDK installed (version 6.0 or as specified in `tests/e2e_csharp_client/E2ETestClient.csproj`). If not, you can typically install it using [Microsoft's official instructions](https://dotnet.microsoft.com/download) or system package managers. The automated environment for this project uses a script to install it if missing.
2.  Run the build command:
    ```bash
    dotnet build tests/e2e_csharp_client/E2ETestClient.csproj
    ```

This will compile the client, making it ready for use by the end-to-end test scripts.
